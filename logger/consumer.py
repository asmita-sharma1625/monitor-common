...
class Consumer:
  ...
  def __init__(self, path, deleterotatedfiles=True, logpattern="", lastfileinfo=None, provider=None):
    self.path = path
#    self.consumerinterval = consumerinterval # Now it seems unneeded.
    ...
    if provider:
      ...
      self.provider = Consumer.DefaultAction()
  ...
  def list_of_consumeables(self, filename):
    ...
    #print "Last file "+lastfilename
    ##print "File list "+`[os.path.join(dirpath, files) for (dirpath,_,filenames) in os.walk(os.path.dirname(filename)) \
    #    for files in filenames if self.regex.search(files) and files > lastfilename]`
    ##print "File list 2 "+`[os.path.join(dirpath, files) for (dirpath,_,filenames) in os.walk(os.path.dirname(filename)) \
